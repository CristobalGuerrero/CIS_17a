...
float  Employee::CalculatePay(float a,int b){
    return (getGrossPay(setHourlyRate(a)
		,setHoursWorked(b)));
}
...
float  Employee::getNetPay(float a){
    if(a<=0){NetPay = 0;}
    else{NetPay=getGrossPay(a,HoursWorked)-Tax(GrossPay);}
    return NetPay;
}
...
int Employee::setHoursWorked(int a){
    ...
    else if(a<=0){cout<<"incorrect hours!\n";HoursWorked = 0;}
    return HoursWorked;
}

float Employee::setHourlyRate(float a){
    if(a>0 || a<=200.00){HourlyRate = a;}
    else{cout<<"incorrect pay rate!\n";HourlyRate = 0;}
    return HourlyRate;
}
